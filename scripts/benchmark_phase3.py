#!/usr/bin/env python3
"""
Phase 3 Benchmarks: Milvus 2.5 Hybrid Search vs Weaviate

This script benchmarks:
1. Milvus 2.5 with hybrid search (dense + sparse)
2. Weaviate 1.27.5 (Phase 2 baseline)

Test categories:
- PDF search (dense, sparse, hybrid)
- Word doc search (dense, sparse, hybrid)
- Image search (dense only - CLIP embeddings)
- Grouping search (Milvus 2.5 only)
"""
...
def setup_milvus_25(pdfs, word_docs, images):
    ...
    pdf_collection = client.create_hybrid_collection("phase3_pdfs_hybrid", dense_dim=384)
    word_collection = client.create_hybrid_collection("phase3_word_hybrid", dense_dim=384)
    image_collection = client.create_hybrid_collection("phase3_images_hybrid", dense_dim=512)
...
def setup_weaviate(pdfs, word_docs, images):
    """Setup Weaviate with Phase 2 collections."""
    client = weaviate.Client("http://localhost:8080")

    print("\n[Weaviate] Creating collections...")
    ...
    # Delete if exists
    try:
        client.schema.delete_class("Phase3PDFs")
    except:
        pass
    ...
    with client.batch as batch:
        for pdf in pdfs:
            ...
            batch.add_data_object(properties, "Phase3PDFs", vector=pdf["embedding"])

    # Create Word collection
    word_schema = {
        "class": "Phase3WordDocs",
        "vectorizer": "none",
        "properties": [
            {"name": "filename", "dataType": ["text"]},
            {"name": "text", "dataType": ["text"]},
    ...
    try:
        client.schema.delete_class("Phase3WordDocs")
    except:
        pass
    ...
    with client.batch as batch:
        for word in word_docs:
            ...
            batch.add_data_object(properties, "Phase3WordDocs", vector=word["embedding"])
    ...
    try:
        client.schema.delete_class("Phase3Images")
    except:
        pass
    ...
    with client.batch as batch:
        for img in images:
            ...
            batch.add_data_object(properties, "Phase3Images", vector=img["image_embedding"])
...
def benchmark_milvus_images(client, collection, images, num_queries=10):
    """Benchmark Milvus 2.5 image search (dense only)."""
    results = {"dense": []}
...
def benchmark_weaviate_pdfs(client, pdfs, num_queries=10):
    ...
    for i in range(num_queries):
        ...
        client.query.get("Phase3PDFs", ["filename", "policy_id", "policy_type"]) \
...
def benchmark_weaviate_word(client, word_docs, num_queries=10):
    """Benchmark Weaviate Word doc search."""
    results = []
    ...
    for i in range(num_queries):
        ...
        start_time = time.time()
        client.query.get("Phase3WordDocs", ["filename", "doc_id", "doc_type"]) \
            .with_near_vector({"vector": query_vector}) \
            .with_limit(5) \
            .do()
        elapsed = (time.time() - start_time) * 1000
...
def benchmark_weaviate_images(client, images, num_queries=10):
    """Benchmark Weaviate image search."""
    results = []
    ...
    for i in range(num_queries):
        ...
        start_time = time.time()
        client.query.get("Phase3Images", ["filename", "claim_id", "damage_type"]) \
            .with_near_vector({"vector": query_vector}) \
            .with_limit(5) \
            .do()
        elapsed = (time.time() - start_time) * 1000
...
def main():
    print("="*70)
    print("Phase 3 Benchmarks: Milvus 2.5 Hybrid Search vs Weaviate")
    print("="*70)
    ...
    results = {
        ...
        "weaviate": {
            "pdf": calculate_stats(weaviate_pdf_results),
            "word": calculate_stats(weaviate_word_results),
            "image": calculate_stats(weaviate_image_results)
        }
    }
    ...
    print(f"  Image:       {results['milvus_25']['image_dense']['avg']:.2f}ms avg")

    print("\nWeaviate Results:")
    print(f"  PDF:   {results['weaviate']['pdf']['avg']:.2f}ms avg")
    print(f"  Word:  {results['weaviate']['word']['avg']:.2f}ms avg")
    print(f"  Image: {results['weaviate']['image']['avg']:.2f}ms avg")

    # Save results
    import os
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_file = Path(os.path.join(script_dir, "..", "results", "phase3_benchmark_results.json"))
    ...
    utility.drop_collection("phase3_pdfs_hybrid")
    utility.drop_collection("phase3_word_hybrid")
    utility.drop_collection("phase3_images_hybrid")

    weaviate_client.schema.delete_class("Phase3PDFs")
    weaviate_client.schema.delete_class("Phase3WordDocs")
    weaviate_client.schema.delete_class("Phase3Images")

    milvus_client.disconnect()

    print("✓ Cleanup complete\n")
    print("="*70)
    print("✓ Phase 3 Benchmarks Complete!")
    print("="*70)
